...
export const resourceService = {
  ...
  getResources: async (params?: PaginationParams): Promise<ApiResponse<Resource>> => {
    ...
    const endpoint = `/resources/${searchParams.toString() ? `?${searchParams.toString()}` : ''}`;
  ...
  getResourcesByType: async (type: string): Promise<ApiResponse<Resource>> => {
    return apiClient.get<ApiResponse<Resource>>(`/resources/?type=${type}`);
  },

  // Get free resources
  getFreeResources: async (): Promise<ApiResponse<Resource>> => {
    return apiClient.get<ApiResponse<Resource>>('/resources/?is_free=true');
  },

  // Get paid resources
  getPaidResources: async (): Promise<ApiResponse<Resource>> => {
    return apiClient.get<ApiResponse<Resource>>('/resources/?is_free=false');
  },

  // Search resources
  searchResources: async (query: string): Promise<ApiResponse<Resource>> => {
    return apiClient.get<ApiResponse<Resource>>(`/resources/?search=${encodeURIComponent(query)}`);
  }
};
